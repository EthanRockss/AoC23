...
int main()
{
	...
	while (getline(input, line))
	{
		...
		bool redposs = maxred >= highest_nums[0];
		bool greenposs = maxgreen >= highest_nums[1];
		bool bluepos = maxblue >= highest_nums[2];
		
		if (redposs && greenposs && bluepos)
		{
			std::cout << gameid << " is possible." << '\n';
			total = total + gameid;
		}
	}
